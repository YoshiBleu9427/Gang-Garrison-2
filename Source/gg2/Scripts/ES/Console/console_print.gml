...
var input, log, maxLength;
input = argument0;
maxLength=global.cMaxLength
// String is too long, break it in pieces and print each on a separate line

while string_length(input) > maxLength{
    var position, tempString, oldPosition, prefix;
 
    position = 0
    oldPosition = 0
    tempString = string_copy(input, 0, maxLength-1)
    ...
    if string_count(" ", tempString) != 0{// Break the line at spaces if possible
        ...
        ds_list_add(global.consoleLog, string_copy(input, 0, maxLength-1));
        prefix = string_copy(input, 0, 3 + COLOR_RGB_LENGTH)
        input = string_copy(input, maxLength, string_length(input));
