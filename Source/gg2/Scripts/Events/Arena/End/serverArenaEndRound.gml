...
while ds_priority_size(redteam) > 5{
    ds_priority_delete_min(redteam);
}

while ds_priority_size(blueteam) > 5{
...
for(i=0; i < redMVPs; i+=1) {
    ...
    write_ubyte(global.sendBuffer, redMVP[i].roundStats[ASSISTS]);
}
...
for(i=0; i < blueMVPs; i+=1) {
    ...
    write_ubyte(global.sendBuffer, blueMVP[i].roundStats[ASSISTS]);
}
