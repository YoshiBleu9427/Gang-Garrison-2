...
if task == 'default' and nearestEnemy != -1 and target != nearestEnemy{
    target = nearestEnemy
...
if nearestEnemy != -1{
    ...
    if point_distance(object.x, object.y, nearestEnemy.x, nearestEnemy.y) < 350{
        ...
        if task != 'default' and target != nearestEnemy and !target_in_sight{
            ds_list_clear(directionList)
            target = nearestEnemy
        }
        // Some random jumping...
